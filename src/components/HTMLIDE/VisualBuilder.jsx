...
const HeadingComponent = ({ text, level, color, textAlign, fontSize }) => {
  const {
...
const ParagraphComponent = ({ text, fontSize, color, fontWeight, textAlign }) => {
  const {
    connectors: { connect, drag },
    actions: { setProp },
    isDragging,
...
const InputComponent = ({ type, placeholder, width }) => {
  const {
    connectors: { connect, drag },
    isDragging,
  } = useNode();
...
const ListComponent = ({ items, ordered, color, fontSize }) => {
  const {
    connectors: { connect, drag },
    actions: { setProp },
    isDragging,
        ...
        {items.map((item, i) => (
          <li
            ...
            onInput={(e) => {
              ...
              setProp((props) => (props.items = newItems));
            }}
          >
            {item}
          </li>
...
ListComponent.craft = {
  displayName: 'List',
  props: {
    items: ['අයිතමය 1', 'අයිතමය 2', 'අයිතමය 3'],
    ordered: false,
    color: '#1f2937',
    fontSize: 16,
  },
...
const DivComponent = ({ backgroundColor, padding, borderRadius, minHeight, borderWidth, borderColor }) => {
  const {
    connectors: { connect, drag },
    isDragging,
  } = useNode();
  ...
  return (
    ...
    >
      <Canvas id="div-canvas" style={{ minHeight: '60px' }} />
    </div>
  );
};
...
const Sidebar = () => {
  ...
  const componentList = [
    ...
    { type: DivComponent, icon: <Square className="w-4 h-4" />, label: 'Div කන්ටේනරය' },
  ];
  ...
  return (
    <div className="w-72 bg-white border-r border-slate-200 p-4 overflow-y-auto flex flex-col">
      ...
      <p className="text-xs text-gray-400 mb-3">අංගයක් එකතු කිරීමට ක්ලික් කරන්න</p>
    ...
    switch (name) {
        ...
        return (
          <>
            ...
            <PropertyField label="අකුරු ප්‍රමාණය (px)">
              <input type="number" value={props.fontSize || 24} onChange={(e) => setProp('fontSize', parseInt(e.target.value) || 24)} className={inputClass} />
            </PropertyField>
      ...
      case 'ParagraphComponent':
        return (
          <>
            <PropertyField label="පෙළ">
              <textarea value={props.text || ''} onChange={(e) => setProp('text', e.target.value)} className={inputClass} rows={3} />
            </PropertyField>
            <PropertyField label="අකුරු ප්‍රමාණය (px)">
      ...
      case 'InputComponent':
        return (
          <>
            <PropertyField label="වර්ගය">
              <select value={props.type || 'text'} onChange={(e) => setProp('type', e.target.value)} className={inputClass}>
            ...
            <PropertyField label="Placeholder">
              <input type="text" value={props.placeholder || ''} onChange={(e) => setProp('placeholder', e.target.value)} className={inputClass} />
            </PropertyField>
            <PropertyField label="පළල (%)">
              <input type="number" value={props.width || 100} onChange={(e) => setProp('width', parseInt(e.target.value) || 100)} className={inputClass} />
            </PropertyField>
      ...
      case 'ListComponent':
        return (
          <>
            <PropertyField label="වර්ගය">
              <select value={props.ordered ? 'ordered' : 'unordered'} onChange={(e) => setProp('ordered', e.target.value === 'ordered')} className={inputClass}>
                <option value="unordered">අක්‍රමිත (•)</option>
                <option value="ordered">අංකිත (1.)</option>
              </select>
            </PropertyField>
            <PropertyField label="වර්ණ">
              <input type="color" value={props.color || '#1f2937'} onChange={(e) => setProp('color', e.target.value)} className="w-full h-8 border border-gray-300 rounded" />
      ...
      case 'DivComponent':
        return (
          <>
            ...
            <PropertyField label="මායිම් ඝණකම (px)">
              <input type="number" value={props.borderWidth || 2} onChange={(e) => setProp('borderWidth', parseInt(e.target.value) || 0)} className={inputClass} />
            </PropertyField>
            <PropertyField label="මායිම් වර්ණ">
              <input type="color" value={props.borderColor || '#9ca3af'} onChange={(e) => setProp('borderColor', e.target.value)} className="w-full h-8 border border-gray-300 rounded" />
            </PropertyField>
          </>
        );
      default:
...
const generateHTMLFromEditor = (query) => {
  ...
  const renderNode = (nodeId) => {
    ...
    switch (name) {
      case 'HeadingComponent':
        return `      <h${props.level} style="color: ${props.color}; text-align: ${props.textAlign}; font-size: ${props.fontSize}px;">${props.text}</h${props.level}>`;
      case 'ParagraphComponent':
      ...
      case 'InputComponent':
        return `      <input type="${props.type}" placeholder="${props.placeholder}" style="width: ${props.width}%; padding: 10px; border: 1px solid #d1d5db; border-radius: 4px;">`;
      case 'ListComponent':
        const tag = props.ordered ? 'ol' : 'ul';
        const items = (props.items || []).map((item) => `        <li>${item}</li>`).join('\n');
        return `      <${tag} style="color: ${props.color}; font-size: ${props.fontSize}px;">\n${items}\n      </${tag}>`;
      case 'DivComponent':
        return `      <div style="background-color: ${props.backgroundColor}; padding: ${props.padding}px; border-radius: ${props.borderRadius}px; min-height: ${props.minHeight}px; border: ${props.borderWidth}px dashed ${props.borderColor};">\n${childHTML}\n      </div>`;
      case 'ROOT':
        return childHTML;
      default:
        return childHTML;
...
<head>
    ...
    <title>Visual Builder පිටුව</title>
    <style>
